...
def calculates_results_stats(results_dic):
    ...
    # Iterate through the results_dic and update counters
    for values in results_dic.values():
        match, is_dog, classifier_is_dog = values[2], values[3], values[4]

        # Count label matches
        n_match += match

        # Count correct dogs
        if is_dog and classifier_is_dog:
            n_correct_dogs += 1
        ...
        # Count correct non-dogs
        if not is_dog and not classifier_is_dog:
            n_correct_notdogs += 1
...
# Create and return results_stats_dic
    results_stats_dic = {
        'n_images': n_images,
